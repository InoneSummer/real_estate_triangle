...
이 public repository는 공개 가능한 수준의 설명과 데모 코드를 제공합니다. 원천 API 키, 내부 운영 로그, 비공개 가공 파일은 포함하지 않습니다.
...
이 저장소는 다음 범위만 공개하는 것을 목표로 합니다.

- 인터랙티브 데모 UI 코드
- 공개 가능한 수준의 synthetic/sample 데이터
- README 기반 방법론 설명
- 데이터 출처와 인용 링크
- 투자 조언이 아니라는 명확한 disclaimer
...
- `.env` 파일
- 공공 API 인증키
- 네이버 API client id/secret
- 텔레그램 봇 토큰과 chat id
- Vercel project metadata
- 원천 실거래 전체 수집 파일
- 내부 운영 로그
- 개인 지식관리 vault 경로
- 개인 데스크톱 경로가 포함된 handoff 문서
- 공개하기 어려운 실험용 prompt와 내부 scoring detail

## 로컬 실행

이 데모는 별도 빌드 없이 정적 파일로 실행할 수 있습니다.
...
정적 웹앱이므로 Vercel에 바로 배포할 수 있습니다.
...
커스텀 도메인은 Vercel 프로젝트에 연결한 뒤 DNS에서 apex와 `www`를 설정합니다.
